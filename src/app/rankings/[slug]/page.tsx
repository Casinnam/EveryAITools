...
export default function RankingsDetailPage({ params }: RankingsDetailPageProps) {
  const { slug } = use(params);
  const { t, language } = useLanguage();

  // For this MVP, we map static rankings slug such as 'best-ai-tools-for-bloggers' to our rich content blog posts
  const post = blogPosts.find(p => p.slug === slug || p.slug === 'best-ai-tools-for-bloggers');
  if (!post) {
    notFound();
  }

  // Pick top 3 tools that fit this ranking category (Writing & Content in tools database)
  const rankedTools = tools
    .filter(t => t.categoryId === 'blog-writing' || t.categoryId === 'marketing-ai')
    .sort((a, b) => b.rating - a.rating)
    .slice(0, 3);

  // FAQ mock data for SEO
  const faqs = [
    {
      ...
      a: {
        ...
        ko: '네, ChatGPT와 Gemini는 회원 가입만 하면 별도 결제 없이도 블로그 기사 작성에 유용한 강력한 무료 버전을 평생 개방해 두고 있습니다.'
      }
    },
    {
      q: { en: 'Which tool is best for bypassing AI detectors?', ko: 'AI 작성 감지 필터를 우회하기에 가장 자연스러운 문체는 무엇인가요?' },
      a: {
        en: 'Claude is widely considered to produce the most human-like sentence flows. Its variation in vocabulary and tone helps write natural articles that rank well on SEO.',
        ko: '현존하는 인공지능 에디터 중에서는 **Claude**가 가장 자연스럽고 작가다운 화풍을 빚어내어, 감지 도구 우회 및 자연스러운 SEO 상위 노출에 가장 뛰어납니다.'
      }
    }
  ];
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8 space-y-12 transition-all duration-300">
      ...
      <article className="prose prose-indigo dark:prose-invert max-w-none rounded-3xl border border-slate-200 bg-white p-6 sm:p-10 shadow-sm dark:border-slate-800 dark:bg-slate-900 leading-relaxed text-sm text-slate-700 dark:text-slate-300 space-y-6">
        <h3 className="text-base font-extrabold text-slate-950 border-l-4 border-indigo-600 pl-3 leading-none dark:text-white">
      ...
      <section className="space-y-6 pt-6 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {faqs.map((faq, i) => (
            ...
            >
              <h4 className="font-extrabold text-slate-950 text-sm flex items-start space-x-2 dark:text-white">
