...
import { Sparkles, HelpCircle, ArrowLeft, ArrowRight, RotateCcw, AlertCircle } from 'lucide-react';
...
export default function ToolFinderPage() {
  ...
  const questions: Question[] = [
    {
      ...
      options: [
        { label: { en: 'Write articles or blog posts', ko: '블로그 포스팅 및 긴 글 쓰기' }, value: 'blog-writing' },
        { label: { en: 'Produce YouTube scripts & edits', ko: '유튜브 시나리오 구상 및 영상 가공' }, value: 'youtube-tools' },
        { label: { en: 'Generate digital art & graphics', ko: '예술 이미지, 일러스트 그래픽 생성' }, value: 'image-generation' },
        { label: { en: 'Create motion video clips & effects', ko: '움직이는 시네마틱 비디오 클립 제작' }, value: 'video-generation-ai' },
        { label: { en: 'Write software code or build websites', ko: '컴퓨터 프로그래밍 및 웹사이트 코딩' }, value: 'coding-ai' },
        { label: { en: 'Draft presentation slides & PPTs', ko: '발표 슬라이드 및 직관적 PPT 문서 제작' }, value: 'presentation-ai' }
      ]
    },
    {
      id: 2,
  ...
  // RECOMMENDATION ENGINE: Calculate matching score (%) for each tool in our database
  const getRecommendedTools = () => {
    ...
    const toolsWithScores = tools.map((tool) => {
      ...
      if (tool.categoryId === quizCategory) {
        score += 40;
      } else if (quizCategory === 'youtube-tools' && tool.categoryId === 'video-generation-ai') {
        // Soft match YouTube creators with Video AI
        score += 25;
      }
      ...
      if (quizSkill === 'beginner') {
        if (tool.beginnerFriendly) score += 15;
        else score += 3; // penalty for pro-only tools for beginners
      } else if (quizSkill === 'professional') {
        if (!tool.beginnerFriendly) score += 15; // Pro users like Cursor, Runway
        else score += 10;
      } else {
      ...
      totalWeights += 20;
      if (quizFree === 'free') {
        if (tool.pricingType === 'Free') score += 20;
        else if (tool.pricingType === 'Freemium') score += 12;
      } else if (quizFree === 'freemium') {
        if (tool.pricingType === 'Free' || tool.pricingType === 'Freemium') score += 20;
      } else {
      ...
      totalWeights += 15;
      if (quizMatters === 'easy' && tool.beginnerFriendly) score += 15;
      else if (quizMatters === 'quality' && tool.rating >= 4.7) score += 15;
      else if (quizMatters === 'korean' && tool.koreanSupport) score += 15;
      else if (quizMatters === 'commercial' && tool.commercialUse) score += 15;
      else score += 5; // default fallback

      // 5. Device preference (Weight: 10 points)
      totalWeights += 10;
      if (quizDevice === 'mobile') {
        if (tool.mobileSupport) score += 10;
        else score += 4;
      } else {
      ...
      {!showResults ? (
        ...
        <section className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-200/80 pb-4 dark:border-slate-800">
            <h3 className="text-lg font-black text-slate-950 dark:text-white flex items-center space-x-2">
          ...
          {recommendedTools.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {recommendedTools.map((tool, index) => (
                <div key={tool.id} className="relative flex flex-col justify-between">
            ...
            <div className="rounded-3xl border border-dashed border-slate-200 bg-white py-16 px-4 text-center dark:border-slate-800 dark:bg-slate-900">
              ...
              <h4 className="mt-4 text-sm font-extrabold text-slate-900 dark:text-white">No Match Found</h4>
